...
//   let priority = axios.get(url + 'api/Employee/GetAllPriority');
//   let response = await priority;
//   console.log(JSON.stringify(response.data.data));
//   let arr = response.data;
//   // const arr.map(({ key: number, value }) => ({ [key]: value }));
//   console.log(response);
// }
export default function ColumnTypesGrid2() {
  //const apiRef = useGridApiContext();
  const apiRef = useGridApiRef();
  const url = "https://localhost:44336/api/Employee/";

  const sendData = () => {
    const a = apiRef.current.getRowModels()
    // const JobId = JSON.parse(localStorage.getItem("JobId") || '{}');
    //const JobId = 1
    //let employeePromise = axios.post(url + `api/Employee/AddRequst?jobId=${JobId}`, rows)
  }
    ...
    () => {
      ...
      axios.get(url + 'GoToDaysNames').then(res => { console.log(res.data); shifrArrDays = res.data }).then(i => {
        axios.get(url + 'GoToGetAllShiftsType').then(res => { console.log(res.data); shifrArrTimes = res.data }).then(x => {
          const pri: Priority[] = JSON.parse(localStorage.getItem("priorityList") || '{}');
          ...
          setColumns(data1)
          let s: RowData = { id: 2, day: "dcfg", shift: [0, 0, 0] }
    ...
    <>
      <div style={{ height: 500, width: '100%' }}>
        <DataGrid columns={columns}
          rows={rows} experimentalFeatures={{ newEditingApi: true }}
